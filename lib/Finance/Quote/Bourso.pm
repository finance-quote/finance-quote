					...
					foreach $row ($ts->rows) {
						...
						ASSIGN:	for ( @$row[0] ) {
								...
								/Valeur liquidative/ && do {
									...
									$info{$stocks, "price"}=$info{$stocks, "last"};
									$info{$stocks, "success"}=1;
									$info{$stocks, "method"}="bourso";
									($info{$stocks, "currency"}=@$row[2]) =~ s/[^A-Z]//g;
									# GnuCash
