...
our $VERSION = '0.1';
my $OnVista_URL = 'http://www.onvista.de/suche/';

sub methods {return (onvista => \&onvista);}
sub labels {return ( onvista => [qw/name last date time currency method symbol exchange/] );}
...
sub onvista {
    ...
    foreach my $stock (@stocks) {
        ...
        if ($reply->is_success) {
            ...
            my @symbolline = $tree->look_down('property', 'schema:productID');
            if (@symbolline) {
                my $symbol = ( $symbolline[0]->content_list )[0];
                ($symbol) = ( $symbol =~ m/(\w+)/ );
                $info{ $stock, "symbol" } = $symbol;
            } else {
                my $symbol = $tree->look_down('data-info-isin', qr/.*/);
		if ($symbol) {
		    $info{ $stock, "symbol" } = $symbol->attr_get_i('data-info-isin');
	        } else {
...
This module fetches information from OnVista, http://www.onvista.de. All stocks are available.
...
terms and conditions See http://www.onvista.de/ for details.

=head1 LABELS RETURNED

The following labels may be returned by Finance::Quote::OnVista :
name, last, date, time, currency, method, exchange, symbol.

=head1 SEE ALSO

OnVista (german web site), http://www.onvista.de
