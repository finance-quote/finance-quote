...
This repo includes updates necessary to add YahooChart and GenericExecutor 
module, which are personally developed and tested. 
...
last seven days of quote data set and then returns last valid data set as 
the valid pricing. The return pricing is always the adjusted pricing that 
accounts for splits and dividends.

This module can also retrieve currency but this seems to be very limited 
...
it. The external program should return data in a stream format. The format 
...
part of a pair is 'ticker', the second part of the pair denotes commodity 
name for which subsequent data pair is associated to it.

See extra folder for a working example of its invocation that calls a Python 
script that uses yfinance library to retrieve information. 
